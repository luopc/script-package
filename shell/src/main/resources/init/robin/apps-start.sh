  ...
  do
    if [[ -f "$APPS_PATH/$app_full_name/instance.profile" ]]; then
      if [[ "$(grep -Fxq "export CURRENT_STATUS=1" "$APPS_PATH/$app_full_name/instance.profile" && echo "true")" = "true" ]]; then
        ...
        start_app "$APPS_PATH" "$app_full_name"
...
start_app() {
  APPS_PATH=$1
  APP_NAME=$2
  if [[ -f "$APPS_PATH/$APP_NAME/start.sh" ]]; then
    echo "                   "
    echo "[$(date +%Y-%m-%d_%H:%M:%S)] [$USER] INFO: Start $APP_NAME ..."
    echo "[$(date +%Y-%m-%d_%H:%M:%S)] [$USER] DEBUG: Running command: sh $APPS_PATH/$APP_NAME/start.sh "

    cd "$APPS_PATH/$APP_NAME" && sh start.sh
